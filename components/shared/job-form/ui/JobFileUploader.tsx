...
// types
import { JobFileUploaderProps } from "@/types/components";
// cmp
...
const JobFileUploader = ({
  image,
  files,
  onFieldChange,
  setImage,
  setFiles,
}: JobFileUploaderProps) => {
  ...
  const onDrop = useCallback(
    ...
    [setFiles, setImage, onFieldChange]
  );
  ...
  return (
    <div className="space-y-5">
      <div {...getRootProps()}>
        ...
        <div
          className={clsx(
            (files.length > 0 || image?.length !== 0) && "aspect-[2/1.2]",
            files.length === 0 &&
              image?.length === 0 &&
...
export default JobFileUploader;
